...
str_y{2}='$\theta_e=\pi/8$';
str_y{3}='$\theta_e=\pi/4$';
str_y{4}='$3\theta_e=\pi/8$';
str_y{5}='$\theta_e=\pi/2$';

[THETA,PSI]=meshgrid(linspace(0,pi/2),linspace(0, 2*pi));
...
for J=1:NJ
    ...
    for I=1:NI
        ...
        if S>-cos(2*theta_e)
            %plot(0,1,'x m',LW{:},MS{:})
            if xc>=0
                plot(cos(xc),0,'s m',LW{:},MS{:})
                % plot(cos(xc-pi/2),0,'o m',LW{:},MS{:})
            else
                plot(cos(xc+pi/2),0,'s m',LW{:},MS{:})
                % plot(cos(xc),0,'o m',LW{:},MS{:})
            end
        else
            plot(0,1,'s m',LW{:},MS{:})
            if xc>=0
                % plot(cos(xc),0,'x m',LW{:},MS{:})
                %plot(cos(xc-pi/2),0,'o m',LW{:},MS{:})
            else
                % plot(cos(xc+pi/2),0,'x m',LW{:},MS{:})
                %plot(cos(xc),0,'o m',LW{:},MS{:})
            end
        end
        if J==1; title(['$S=',num2str(S),'$'],TX{:},FS{:}); end        
        if I==1; ylabel(str_y{J},TX{:},FS{:}); end
        ax.Title.Visible='on';
...
title(t1,title_str,TX{:},FS{:})
ylabel(t1,'Angle of maximum extension',TX{:},FS{:})
cbar=colorbar;
...
print('../ReactiveShearInstability-Manuscript/figure-kx-ky.eps','-depsc')
